...
<h2>Lịch sử đăng ký/hủy</h2>
<?php 
$this->widget('bootstrap.widgets.TbGridView',array(
	'id'=>'subscriber-transaction-grid',
...
// 		),
        array(
            ...
            'type' => 'raw',
            'value' => function ($data) {
                $idsub = $data->subscriber_id;
                $category = ServiceSubscriberMapping::model()->findBySql("select * from service_subscriber_mapping where subscriber_id = ".$idsub);
                if($category != null){
		...
		array(
				'name'=>'vnp_username',
				'value'=>'$data->getVnpUsername()',
				// 			'value'=>$model->html_content,
				'htmlOptions'=>array('class'=>'html', 'rows'=>1,'height'=>'30px', 'width'=>'70px', 'style' => 'max-height:50px;'),
		),
		array(
				'name'=>'vnp_ip',
				'value'=>'$data->getVnpUserIp()',
				// 			'value'=>$model->html_content,
				'htmlOptions'=>array('class'=>'html', 'rows'=>1,'height'=>'30px', 'width'=>'70px', 'style' => 'max-height:50px;'),
		),
// 		array(
