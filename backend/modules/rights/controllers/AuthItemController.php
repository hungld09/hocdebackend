...
class AuthItemController extends RController
{
	...
	*/
	public function actionUpdate()
	{
		...
		{
			$formModel->attributes = $_POST['AuthItemForm'];
			$formModel->level = $_POST['AuthItemForm']['level'];
// 			echo $formModel->level.'___LEVEL'; exit;
			if( $formModel->validate()===true )
			{
				// Update the item and load it
				$this->_authorizer->updateAuthItem($itemName, $formModel->name, $formModel->description, $formModel->bizRule, $formModel->data, $formModel->level);
		...
		$formModel->level = $model->level!==null ? $model->level : '';
		...
		// Render the view
		if(DEPLOY_STATUS == 1) {
			$this->render('updateDeploy', array(
					'model'=>$model,
					'formModel'=>$formModel,
