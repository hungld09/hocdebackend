...
// 		$currentItemName = self::get1stAssignedRole($userId)->name;
// 		self::revoke($currentItemName, $userId);
	...
	*/
	public static function getAssignedRoles($userId=null, $sort=true)
	{
		...
		if( $userId===null && $user->isGuest===false )
	 	...
	 	$authorizer = self::getAuthorizer();
	 	$result = $authorizer->getAuthItems(CAuthItem::TYPE_ROLE, $userId, null, $sort);
// 	 	print_r($result); exit;
	 	return $result;
	}

	//Chau added
	//lay role object cua user co id la userId
	public static function get1stAssignedRole($userId=null, $sort=true)
	{
		$user = Yii::app()->getUser();
		if( $userId===null && $user->isGuest===false )
			$userId = $user->id;
	 	...
	 	$authorizer = self::getAuthorizer();
	 	$role = $authorizer->get1stAuthItem(CAuthItem::TYPE_ROLE, $userId, null, $sort);
// 	 	return NULL;
	 	return $role;
	}
