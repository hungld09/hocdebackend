	...
	*/
	public function getAuthItems($type=null, $userId=null, $sort=true)
	{
		...
		if( $sort===true )
		{
			...
			{
				$sql = "SELECT name,t1.type,description,t1.bizrule,t1.data, t1.level ,weight
			...
			foreach($command->queryAll() as $row)
				$items[ $row['name'] ] = new CAuthItem($this, $row['name'], $row['type'], $row['description'], $row['bizrule'], unserialize($row['data']), $row['level']);
		}
		// No sorting required.
		else
		{
	...
	public function getItemChildren($names, $allowCaching=true)
	{
		...
		else
		{
			...
			$sql = "SELECT name, type, description, bizrule, data, level
