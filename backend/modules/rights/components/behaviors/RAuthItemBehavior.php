...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	...
	/**
	* Returns the markup for the level link to displayed in the grid.
	* @return string the markup. 
	*/
	public function getGridLevelLink()
	{
// 		$markup = CHtml::link($this->owner->level, array(
// 			'authItem/update',
// 			'level'=>urlencode($this->owner->level),
// 		));
		$markup = intval($this->owner->getLevel());
// 		$markup .= $this->childCount();
// 		$markup .= $this->sortableId();
	...
	*/
	...
	{
		return CHtml::linkButton(Rights::t('core', 'Hủy quyền'), array(
